...
public interface ClaudeOsrsConfig extends Config
{
    ...
    @ConfigItem(
        keyName = "authToken",
        name = "Auth Token",
        description = "Optional secret token. If set, all MCP requests must include it as a Bearer token. Leave blank to disable auth.",
        section = connectionSection,
        position = 1,
        secret = true
    )
    ...
    @ConfigSection(
        name = "Privacy",
        description = "Control what data Claude can access",
        position = 1
    )
